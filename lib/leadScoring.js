...
export function scoreShipmentActivity(shipmentCount) {
...
export function scoreTradeValue(totalShipmentValue) {
...
export function scoreProductMatch(productMatch) {
  return productMatch ? MAX_PRODUCT_MATCH_POINTS : 0;
}

export function scoreCountryMatch(countryMatch) {
  return countryMatch ? MAX_COUNTRY_MATCH_POINTS : 0;
}

export function scoreVerification(verified) {
  return verified ? MAX_VERIFICATION_POINTS : 0;
}

export function scoreRecency(lastShipmentDate, asOfDate) {
 ...
 */
export function computeLeadScore(entity, context = {}) {
