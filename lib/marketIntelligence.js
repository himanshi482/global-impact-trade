...
import { confidenceFromMetrics } from "./dataConfidence";
import { getTradeDataProvider } from "./tradeDataProvider";
import { getTradeTrends } from "./tradeTrends";
...
export async function findBestMarkets({ hsCode, product = null, limit = 6 }) {
  ...
  if (hsCode) {
    buyerWhere.push("(hs_code LIKE ? OR hs_chapter = ?)");
    buyerParams.push(`%${hsCode}%`, hsCode.slice(0, 2));

    shipmentWhere.push("hs_code LIKE ?");
    shipmentParams.push(`%${hsCode}%`);
  }
  if (product) {
    buyerWhere.push("product LIKE ?");
    buyerParams.push(`%${product}%`);

    shipmentWhere.push("product LIKE ?");
    shipmentParams.push(`%${product}%`);
  }

  const bWhereClause = buyerWhere.length ? `WHERE ${buyerWhere.join(" AND ")}` : "";
  const shWhereClause = shipmentWhere.length ? `WHERE ${shipmentWhere.join(" AND ")}` : "";

  const [buyerCountries, shipmentCountries] = await Promise.all([
    query(
      `SELECT DISTINCT country FROM buyers ${bWhereClause} ${bWhereClause ? 'AND' : 'WHERE'} country IS NOT NULL AND country != ''`,
      buyerParams
    ),
    query(
      `SELECT DISTINCT destination_country AS country FROM shipments ${shWhereClause} ${shWhereClause ? 'AND' : 'WHERE'} destination_country IS NOT NULL AND destination_country != ''`,
      shipmentParams
    ),
  ]);
...
export async function getUnifiedMarketAnalysis({
  ...
  const [metrics, trendResult, topBuyers, topSuppliers, shipmentIntel, historicalTrend] = await Promise.all([
    ...
    getTradeTrends({ hsCode, country, direction, granularity: "monthly" }),
  ]);
  ...
  const dataConfidence = confidenceFromMetrics(metrics, {
    source: getTradeDataProvider().source,
    freshness: dataQuality.lastRefreshed,
    limitations: trendResult.hasData ? [] : ["Insufficient historical data"],
  });
