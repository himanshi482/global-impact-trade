  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <main className="mx-auto max-w-6xl px-4 py-8">
        <h1 className="text-2xl font-semibold text-yellow-400 mb-1">Buyer Discovery</h1>
        <p className="text-sm text-neutral-400 mb-6">
          Search real buyer records from GlobeBridge trade data.
        </p>

        <FilterBar filters={filters} onChange={handleFilterChange} />

        {status === 'loading' && <StateMessage text="Loading buyers…" />}
        {status === 'error' && (
          <StateMessage text="Something went wrong loading buyers. Please try again." isError />
        )}
        {status === 'success' && results.length === 0 && (
          <StateMessage text="No buyers match your filters yet. Try broadening your search." />
        )}
    ...
    <div className="border border-neutral-800 rounded-lg p-4 bg-neutral-950">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-xs text-neutral-400">{buyer.country}</p>
        </div>
        {buyer.verified && (
      ...
      <div className="mt-3 text-sm text-neutral-300 space-y-1">
        <p>Product: {buyer.product}</p>
        <p>HS Code: {buyer.hsCode}</p>
        <p>
          Shipments: {buyer.shipmentCount} · Value: $
          {Number(buyer.totalShipmentValue).toLocaleString()}
        </p>
      </div>

      <div className="mt-3 flex items-center justify-between">
        <span className={`text-sm font-semibold ${potentialColor}`}>
          Lead Score: {buyer.leadScore}/100
        </span>
        <button
          onClick={onSave}
          disabled={saving}
