...
export default function BuyerProfilePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // GET /api/buyers/[id]. This assumes it returns { buyer, shipments }
  ...
  const handleUnlock = async () => {
    ...
    try {
      const res = await fetch('/api/unlock', {
        ...
        body: JSON.stringify({ entityType: 'BUYER', entityId: id }),
      });
      if (res.status === 403) {
  ...
  if (status === 'loading') {
    return <PageShell><p className="text-neutral-400">Loading buyer profile…</p></PageShell>;
  }
  if (status === 'error' || !buyer) {
    return (
      <PageShell>
        <p className="text-red-400">Could not load this buyer profile. Please try again.</p>
      </PageShell>
    );
  }
  ...
  return (
    <PageShell>
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-white">{buyer.companyName}</h1>
          <p className="text-neutral-400">{buyer.country}</p>
        </div>
        <button
      ...
      <section className="grid gap-6 md:grid-cols-3">
        <div className="md:col-span-2 space-y-6">
          <div className="border border-neutral-800 rounded-lg p-4">
            ...
            <dl className="grid grid-cols-2 gap-y-2 text-sm text-neutral-300">
              ...
              <dd>{buyer.shipmentCount}</dd>
              <dt className="text-neutral-500">Total Trade Value</dt>
              <dd>${Number(buyer.totalShipmentValue).toLocaleString()}</dd>
              <dt className="text-neutral-500">Verified</dt>
              <dd>{buyer.verified ? 'Yes' : 'No'}</dd>
            </dl>
          </div>

          <div className="border border-neutral-800 rounded-lg p-4">
            <h2 className="font-medium text-white mb-3">
              Lead Score: {buyer.leadScore}/100 — {buyer.potential}
            </h2>
            <ul className="text-sm text-neutral-300 space-y-1">
              {(buyer.scoreReasons || []).map((reason, i) => (
                <li key={i}>✓ {reason}</li>
              ))}
            </ul>
          </div>
        ...
        <aside className="border border-neutral-800 rounded-lg p-4 h-fit">
          ...
          {buyer.contactUnlocked ? (
            <dl className="text-sm text-neutral-300 space-y-2">
              <div><dt className="text-neutral-500">Email</dt><dd>{buyer.email}</dd></div>
              <div><dt className="text-neutral-500">Phone</dt><dd>{buyer.phone}</dd></div>
              <div><dt className="text-neutral-500">Contact Person</dt><dd>{buyer.contactPerson}</dd></div>
              <div><dt className="text-neutral-500">Website</dt><dd>{buyer.website}</dd></div>
            </dl>
          ) : (
